...
(async function() {
    ...
    GM_addStyle(`
        ...
        #migaku-controller.minimized #migaku-toggle-btn { top: 50%; left: 50%; right: auto; transform: translate(-50%, -50%); }
    ...
    function logToPopup(message) { if (state.isEmbedded) return; const logEntry=`[${new Date().toLocaleTimeString()}] ${message}`; if (!state.logElement && state.controller) state.logElement = state.controller.querySelector('.log-area'); if (state.logElement) { const entry=document.createElement('div'); entry.textContent=logEntry; state.logElement.appendChild(entry); state.logElement.scrollTop=state.logElement.scrollHeight; } else { state.logBuffer.push(logEntry); } console.log(message); }
...
function updateActiveSavedDisplay() { if(state.isEmbedded)return;const anDisp=document.getElementById('active-saved-anime-display'),imDisp=document.getElementById('active-import-display');if(anDisp)anDisp.textContent=state.activeSavedAnimeKey||'N/A';if(imDisp){if(state.activeSavedAnimeKey&&state.activeImportId){const sess=state.savedAnimeData[state.activeSavedAnimeKey]?.find(s=>s.importId===state.activeImportId);imDisp.textContent=sess?`${sess.importName} (${sess.dateAdded.substring(0,10)})`:'N/A';}else imDisp.textContent='N/A';}}
    ...
    function updateDetectionDisplay() { if(state.isEmbedded)return;const animeEl=document.getElementById('detected-anime-name');if(animeEl)animeEl.textContent=state.detectedAnimeName||'N/A';const epEl=document.getElementById('detected-episode');if(epEl)epEl.textContent=state.detectedEpisode!==null?state.detectedEpisode:'N/A';if(state.logElement&&state.logBuffer.length>0){state.logBuffer.forEach(t=>{const e=document.createElement('div');e.textContent=t;state.logElement.appendChild(e);});state.logElement.scrollTop=state.logElement.scrollHeight;state.logBuffer=[];}}
    ...
    function detectAnimeName() { pageDetectAnimeName(state); }
    function detectEpisode() { pageDetectEpisode(state); }
